...
function initPriceCalculator(basePrice) {
    ...
    const calculate = () => {
        ...
        let discount = 1, discountChildren=1;
        let discountText='';
        let total = 0;
        if(totalPeople>20){
            discount=0.96;
            //total -= basePrice;
            discountText += `+1 билет в подарок<br>`;
        }
        else if(totalPeople>=10){
            discount=0.97;
        }
        else if(children>0||pensioners>0){
            discountChildren=0.98;
        }
        total += adults* basePrice * discount + (children + pensioners) * basePrice * discountChildren;
        document.getElementById('discount').innerHTML = `-${(100-discount*100).toFixed(0)}% ${discountText}`;
